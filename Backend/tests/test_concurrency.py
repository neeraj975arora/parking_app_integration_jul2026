        ...
        if response.status_code == 201:
            ...
            def read_operation(thread_id):
                ...
                try:
                    ...
                    # Flask's test client is not thread-safe. Give every worker
                    # an independent client while keeping the same application
                    # and database, matching concurrent real HTTP requests.
                    thread_client = client.application.test_client()
                    response = thread_client.get(endpoint, headers=auth_headers)
